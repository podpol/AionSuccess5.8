...
public class MySQL5AccountDAO extends AccountDAO {
	...
	public Account getAccount(String name) {
		...
		try {
			...
			if (rs.next()) {
				...
				account.setPrestigeSociety(rs.getTimestamp("prestige"));
				account.setVipLevel(rs.getInt("vip_level"));
                account.setVipEffect(rs.getString("vip_effects"));
                account.setVipTime(rs.getTimestamp("vip_time"));
			}
		}
		catch (Exception e) {
	...
	public boolean updateAccount(Account account) {
		int result = 0;
		PreparedStatement st = DB
			.prepareStatement("UPDATE account_data SET `name` = ?, `password` = ?, access_level = ?, membership = ?, last_server = ?, last_ip = ?, last_mac = ?, ip_force = ?, return_account = ?, return_end = ?, vip_level = ?, vip_effects = ?, vip_time = ? WHERE `id` = ?");
		...
		try {
			...
			st.setInt(10, account.getVipLevel());
            st.setString(11, account.getVipEffect());
            st.setTimestamp(12, account.getVipTime());
            log.info("Vip TIme: " + account.getVipTime());
