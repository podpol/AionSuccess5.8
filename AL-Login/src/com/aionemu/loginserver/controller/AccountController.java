...
public class AccountController {
    ...
    public static synchronized void checkAuth(SessionKey key, GsConnection gsConnection) {
        ...
        if (con != null && con.getSessionKey().checkSessionKey(key)) {
             ...
             */
            try {
            	gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, true, acc.getName(), acc.getAccessLevel(), acc.getMembership(), toll, luna, acc.getReturn(), acc.getPrestigeSociety().getTime(), acc.getVipLevel(), acc.getVipEffect(), acc.getVipTime().getTime()));
            } catch (NullPointerException e) {
            	gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, true, acc.getName(), acc.getAccessLevel(), acc.getMembership(), toll, luna, acc.getReturn(), 0, 0, "", 0));
            }
        } else {
            gsConnection.sendPacket(new SM_ACCOUNT_AUTH_RESPONSE(key.accountId, false, null, (byte) 0, (byte) 0, 0, 0, (byte)0, 0, 0, "", 0));
        }
    }
