...
public class _25670Investigating_The_Ancient_Guardian_Weapon_Invasion extends QuestHandler
{
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			} if (targetId == 703439) { //DF6_FOBJ_Od_Track_Q25670_A
                switch (env.getDialog()) {
                    case USE_OBJECT: {
                        return closeDialogWindow(env);
					}
                }
            } if (targetId == 703440) { //DF6_FOBJ_Od_Track_Q25670_B
                switch (env.getDialog()) {
                    case USE_OBJECT: {
                        return closeDialogWindow(env);
					}
                }
            } if (targetId == 703441) { //DF6_FOBJ_Od_Track_Q25670_C
                switch (env.getDialog()) {
                    case USE_OBJECT: {
                        return closeDialogWindow(env);
					}
                }
            } if (targetId == 703442) { //DF6_FOBJ_Od_Track_Q25670_D
                switch (env.getDialog()) {
                    case USE_OBJECT: {
                        return closeDialogWindow(env);
					}
                }
            } if (targetId == 731794) { //DF6_FOBJ_Od_Track_Q25670_E
                switch (env.getDialog()) {
				    case USE_OBJECT: {
						if (var == 2) {
