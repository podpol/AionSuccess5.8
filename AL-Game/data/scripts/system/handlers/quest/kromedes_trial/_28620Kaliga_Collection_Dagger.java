...
public class _28620Kaliga_Collection_Dagger extends QuestHandler {

	private final static int questId = 28620;
	
	public _28620Kaliga_Collection_Dagger() {
		super(questId);
	}
	
	@Override
	public void register() {
		qe.registerQuestNpc(730328).addOnQuestStart(questId); //Kaliga's Dagger Rack.
		qe.registerQuestNpc(730328).addOnTalkEvent(questId); //Kaliga's Dagger Rack.
	}
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		if (targetId == 730328) { //Kaliga's Dagger Rack.
			...
			if ((playerClass == PlayerClass.WARRIOR || playerClass == PlayerClass.SCOUT ||
				...
				playerClass == PlayerClass.RANGER) && player.getCommonData().getRace() == Race.ASMODIANS) {
				if (qs == null || qs.getStatus() == QuestStatus.NONE || qs.canRepeat()) {
