...
public class _20011A_Flower_That_Blooms_With_Death extends QuestHandler
{
    ...
    public boolean onLvlUpEvent(QuestEnv env) {
        return defaultOnLvlUpEvent(env, 20010, true);
    }
	...
	public boolean onDialogEvent(QuestEnv env) {
        ...
        if (env.getVisibleObject() instanceof Npc) {
			...
			} if (targetId == 731788) { //IDAb1_Ere_Dead_Deva_D_E.
				switch (env.getDialog()) {
					case USE_OBJECT: {
						if (var == 1) {
			...
			} if (targetId == 731789) { //IDAb1_Ere_Dying_Deva_D_E.
				switch (env.getDialog()) {
					case USE_OBJECT: {
						if (var == 2) {
			...
			} if (targetId == 731790) { //IDAb1_Ere_Pain_Deva_D_E.
				switch (env.getDialog()) {
					case USE_OBJECT: {
						if (var == 3) {
			...
			} if (targetId == 806598) { //IDAb1_Ere_Terrarium_End_Vatiskhan_E.
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (var == 8) {
