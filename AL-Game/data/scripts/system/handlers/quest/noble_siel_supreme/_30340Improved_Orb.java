...
public class _30340Improved_Orb extends QuestHandler
{
	private final static int questId = 30340;
	
	public _30340Improved_Orb() {
		super(questId);
	}
	...
	public void register() {
		...
		qe.registerGetingItem(182209738, questId);
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799336) { //Tataka.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182209738) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182209738, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182209738, 1);
					}
				}
			}
		}
