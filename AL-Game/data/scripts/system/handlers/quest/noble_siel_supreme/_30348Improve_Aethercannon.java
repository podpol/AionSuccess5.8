...
public class _30348Improve_Aethercannon extends QuestHandler
{
	private final static int questId = 30348;
	
	public _30348Improve_Aethercannon() {
		super(questId);
	}
	...
	public void register() {
		...
		qe.registerGetingItem(182213289, questId);
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799336) { //Tataka.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182213289) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182213289, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182213289, 1);
					}
				}
			}
		}
