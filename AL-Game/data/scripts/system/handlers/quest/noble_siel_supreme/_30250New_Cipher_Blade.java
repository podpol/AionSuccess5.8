...
public class _30250New_Cipher_Blade extends QuestHandler
{
	private final static int questId = 30250;
	
	public _30250New_Cipher_Blade() {
		super(questId);
	}
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799032) { //Gefeios.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182213559) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182213559, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182213559, 1);
					}
				}
			}
		}
