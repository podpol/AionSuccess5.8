...
public class _30248New_Aethercannon extends QuestHandler
{
	private final static int questId = 30248;
	
	public _30248New_Aethercannon() {
		super(questId);
	}
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799032) { //Gefeios.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182213286) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182213286, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182213286, 1);
					}
				}
			}
		}
