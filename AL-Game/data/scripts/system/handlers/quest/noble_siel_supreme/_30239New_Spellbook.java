...
public class _30239New_Spellbook extends QuestHandler
{
	private final static int questId = 30239;
	
	public _30239New_Spellbook() {
		super(questId);
	}
	...
	public void register() {
		...
		qe.registerGetingItem(182209637, questId);
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799032) { //Gefeios.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182209637) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182209637, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182209637, 1);
					}
				}
			}
		}
