...
public class _30243New_Mace extends QuestHandler
{
	private final static int questId = 30243;
	
	public _30243New_Mace() {
		super(questId);
	}
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE) {
			...
			if (targetId == 799032) { //Gefeios.
				if (dialog == QuestDialog.USE_OBJECT) {
					if (player.getInventory().getItemCountByItemId(182209641) > 0) {
						return sendQuestDialog(env, 10002);
					}
				} else {
					removeQuestItem(env, 182209641, 1);
			...
			switch (targetId) {
				case 215795: { //Debilkarim The Maker.
					if (QuestService.collectItemCheck(env, true)) {
						return giveQuestItem(env, 182209641, 1);
					}
				}
			}
		}
