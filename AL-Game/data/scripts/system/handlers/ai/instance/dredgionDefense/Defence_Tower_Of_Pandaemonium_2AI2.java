...
@AIName("Defence_Tower_Of_Pandaemonium_2")
public class Defence_Tower_Of_Pandaemonium_2AI2 extends ActionItemNpcAI2
{
	...
	protected void handleAttack(Creature creature) {
		...
		if (isAggred.compareAndSet(false, true)) {
			switch (getNpcId()) {
				case 221139: //Defence Tower Of Pandaemonium 2.
				    announceDefencePandaemoniumB();
				break;
			}
		}
	}
	...
	private void dreadgionDrakanATK() {
		dreadgionDrakanATKTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new Runnable() {
			@Override
			public void run() {
				AI2Actions.targetCreature(Defence_Tower_Of_Pandaemonium_2AI2.this, getPosition().getWorldMapInstance().getNpc(220967));
                AI2Actions.useSkill(Defence_Tower_Of_Pandaemonium_2AI2.this, 18311);
			}
		}, 4000, 10000);
	}
	...
	private void startDreadgionOverheatTask() {
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				//AI2Actions.useSkill(Defence_Tower_Of_Pandaemonium_2AI2.this, 18310); //Dreadgion Overheat.
			}
		}, 300000);
	}
	
	private void announceDefencePandaemoniumB() {
	...
	private void announceDefencePandaemoniumDieB() {
		getPosition().getWorldMapInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
	...
	protected void handleDied() {
		announceDefencePandaemoniumDieB();
		super.handleDied();
	}
