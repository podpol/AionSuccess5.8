...
@AIName("WindStream_05_Li")
public class WindStream_05_LiAI2 extends NpcAI2
{
	@Override
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                Npc npc2 = (Npc) spawn(302200000, 220970, 1569.3729f, 1484.9878f, 573.33221f, (byte) 0, 0, 1);
				windStreamAnnounce(npc2, 1);
				despawnNpc(220973);
                spawn(220970, 1569.3729f, 1484.9878f, 573.33221f, (byte) 0, 762);
                PacketSendUtility.broadcastPacket(npc2, new SM_WINDSTREAM_ANNOUNCE(1, 302200000, 362, 1));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302200000, 362, state));
            }
        });
    }
