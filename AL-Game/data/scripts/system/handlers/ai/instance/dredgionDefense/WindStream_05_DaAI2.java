...
@AIName("WindStream_05_Da")
public class WindStream_05_DaAI2 extends NpcAI2
{
	@Override
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
				...
				despawnNpc(220981);
                spawn(220970, 1245.9900f, 1319.3500f, 204.75000f, (byte) 0, 1284);
                PacketSendUtility.broadcastPacket(npc2, new SM_WINDSTREAM_ANNOUNCE(1, 302300000, 369, 1));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302300000, 369, state));
            }
        });
    }
