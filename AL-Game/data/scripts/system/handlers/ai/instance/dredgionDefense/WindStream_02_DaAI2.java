...
@AIName("WindStream_02_Da")
public class WindStream_02_DaAI2 extends NpcAI2
{
	@Override
	...
	private void startWindStream(final Npc npc) {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                Npc npc2 = (Npc) spawn(302300000, 220970, 1328.5240f, 1360.0000f, 208.47737f, (byte) 0, 0, 1);
				windStreamAnnounce(npc2, 1);
				despawnNpc(220975);
                spawn(220970, 1328.5240f, 1360.0000f, 208.47737f, (byte) 0, 1271);
                PacketSendUtility.broadcastPacket(npc2, new SM_WINDSTREAM_ANNOUNCE(1, 302300000, 368, 1));
                if (npc2 != null) {
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
                PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302300000, 368, state));
            }
        });
    }
