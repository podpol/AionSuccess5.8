...
public class Runaway_HirakikiAI2 extends GeneralNpcAI2
{
		...
		if (isAggred.compareAndSet(false, true)) {
			switch (getNpcId()) {
				case 235763: //Runaway Hirakiki Leader.
					...
					hirakikiTask = ThreadPoolManager.getInstance().schedule(new Runnable() {
						@Override
						public void run() {
							AI2Actions.deleteOwner(Runaway_HirakikiAI2.this);
						}
					}, 480000);
				break;
			}
	...
	protected void handleCreatureMoved(Creature creature) {
		...
		if (getNpcId() == 235781 && isInRange(creature, 15) && creature instanceof Player) {
		...
		} else if (getNpcId() == 235760 && isInRange(creature, 15) && creature instanceof Player) {
			if (startedEvent.compareAndSet(false, true)) {
				switch (Rnd.get(1, 2)) {
					case 1:
	...
	private void cancelHirakikiTask() {
		if (hirakikiTask != null && !hirakikiTask.isDone()) {
			hirakikiTask.cancel(true);
		}
	}
