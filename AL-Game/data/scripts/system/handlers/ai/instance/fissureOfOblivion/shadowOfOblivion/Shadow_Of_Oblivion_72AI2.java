...
@AIName("Shadow_Of_Oblivion_72")
public class Shadow_Of_Oblivion_72AI2 extends AggressiveNpcAI2
{
	@Override
	...
	private void ShadowOfOblivionType() {
		...
		switch (Rnd.get(1, 4)) {
			...
			break;
			case 2:
				spawn(244738, getOwner().getX(), getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
			break;
			case 3:
				spawn(244739, getOwner().getX(), getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
			break;
			case 4:
				spawn(244740, getOwner().getX(), getOwner().getY(), getOwner().getZ(), (byte) getOwner().getHeading());
			break;
		}
	}
}