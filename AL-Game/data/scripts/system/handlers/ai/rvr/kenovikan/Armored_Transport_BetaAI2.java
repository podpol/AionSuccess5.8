...
@AIName("Armored_Transport_Beta")
public class Armored_Transport_BetaAI2 extends GeneralNpcAI2
{
	...
	private void announceF6RaidSumKill02DarkDie() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
					//Armored Transport Beta was destroyed, and some users were given a special effect.
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1404268));
				}
			}
		});
	}
	...
	protected void handleDied() {
		removeF6RewardTrans();
		announceF6RaidSumKill02DarkDie();
		super.handleDied();
	}
}