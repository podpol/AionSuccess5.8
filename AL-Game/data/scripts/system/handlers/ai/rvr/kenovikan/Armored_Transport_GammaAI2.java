...
@AIName("Armored_Transport_Gamma")
public class Armored_Transport_GammaAI2 extends GeneralNpcAI2
{
    private boolean canThink = true;
	private String walkerId = "220110003";
		...
		if (creature instanceof Player) {
			...
			if (MathUtil.getDistance(getOwner(), player) <= 100) {
				if (startedEvent.compareAndSet(false, true)) {
					canThink = false;
					getSpawnTemplate().setWalkerId("220110003");
	...
	private void announceF6RaidSumKill03DarkDie() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
					//Armored Transport Gamma was destroyed, and some users were given a special effect.
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1404269));
				}
			}
		});
	}
	...
	protected void handleDied() {
		removeF6RewardTrans();
		announceF6RaidSumKill03DarkDie();
		super.handleDied();
	}
}