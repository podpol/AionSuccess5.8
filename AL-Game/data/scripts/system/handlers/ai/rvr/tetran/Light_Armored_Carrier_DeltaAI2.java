...
public class Light_Armored_Carrier_DeltaAI2 extends GeneralNpcAI2
{
		...
		if (creature instanceof Player) {
			...
			if (MathUtil.getDistance(getOwner(), player) <= 100) {
				if (startedEvent.compareAndSet(false, true)) {
					canThink = false;
					getSpawnTemplate().setWalkerId("210100004");
	...
	private void announceF6RaidSumKill04LightDie() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
					//Light Armored Carrier Delta was destroyed, and some users were given a special effect.
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1404273));
				}
			}
		});
	}
	...
	protected void handleDied() {
		removeF6RewardTrans();
		announceF6RaidSumKill04LightDie();
		super.handleDied();
	}
}