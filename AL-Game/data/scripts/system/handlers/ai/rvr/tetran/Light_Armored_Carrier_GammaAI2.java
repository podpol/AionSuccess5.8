...
@AIName("Light_Armored_Carrier_Gamma")
public class Light_Armored_Carrier_GammaAI2 extends GeneralNpcAI2
{
    private boolean canThink = true;
	private String walkerId = "210100003";
		...
		if (creature instanceof Player) {
			...
			if (MathUtil.getDistance(getOwner(), player) <= 100) {
				if (startedEvent.compareAndSet(false, true)) {
					canThink = false;
					getSpawnTemplate().setWalkerId("210100003");
	...
	private void announceF6RaidSumKill03LightDie() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
					//Light Armored Carrier Gamma was destroyed, and some users were given a special effect.
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1404272));
				}
			}
		});
	}
	...
	protected void handleDied() {
		removeF6RewardTrans();
		announceF6RaidSumKill03LightDie();
		super.handleDied();
	}
}