...
public class Light_Armored_Carrier_BetaAI2 extends GeneralNpcAI2
{
    private boolean canThink = true;
	private String walkerId = "210100002";
		...
		if (creature instanceof Player) {
			...
			if (MathUtil.getDistance(getOwner(), player) <= 100) {
				if (startedEvent.compareAndSet(false, true)) {
					canThink = false;
					getSpawnTemplate().setWalkerId("210100002");
	...
	private void announceF6RaidSumKill02LightDie() {
		World.getInstance().doOnAllPlayers(new Visitor<Player>() {
			@Override
			public void visit(Player player) {
				if (player.isOnline()) {
					//Light Armored Carrier Beta was destroyed, and some users were given a special effect.
					PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1404271));
				}
			}
		});
	}
	...
	protected void handleDied() {
		removeF6RewardTrans();
		announceF6RaidSumKill02LightDie();
		super.handleDied();
	}
}