...
public class Chromaleaf_FlowershotAI2 extends AggressiveNpcAI2
{
					...
					if (!players.isEmpty()) {
						...
						if (players.size() < 6) {
							...
							for (int i = 0; i < count; i++) {
								...
								spawnExplodingPearl(players.get(Rnd.get(players.size())));
							}
						}
					}
				}
	...
	private void spawnExplodingPearl(Player player) {
		...
		if (x > 0 && y > 0 && z > 0) {
			ThreadPoolManager.getInstance().schedule(new Runnable() {
				...
				public void run() {
					if (!isAlreadyDead()) {
						spawn(235964, x, y, z, (byte) 0); //Exploding Pearl.
					}
				}
			}, 3000);
		}
	...
	protected void handleDied() {
		...
		if (p != null) {
			deleteNpcs(p.getWorldMapInstance().getNpcs(235964)); //Exploding Pearl.
		}
		cancelPhaseTask();
		super.handleDied();
	}
