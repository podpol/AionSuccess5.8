	...
	public void storePlayer(final Player player) {
		...
		try {
			...
			PreparedStatement stmt = con
				.prepareStatement("UPDATE players SET name=?, exp=?, recoverexp=?, x=?, y=?, z=?, heading=?, world_id=?, gender=?, race=?, player_class=?, last_online=?, quest_expands=?, npc_expands=?, advenced_stigma_slot_size=?, warehouse_size=?, note=?, title_id=?, bonus_title_id=?, dp=?, soul_sickness=?, mailbox_letters=?, reposte_energy=?, mentor_flag_time=?, world_owner=?, stamps=?, rewarded_pass=?, last_stamp=?, passport_time=?, is_archdaeva=?, creativity_point=?, aura_of_growth=?, join_legion_id=?, join_state=?, berdin_star=?, abyss_favor=?, luna_consume=?, muni_keys=?, luna_consume_count=?, wardrobe_slot=?, frenzy_points=?, frenzy_count=?, toc_floor=?, minion_energy=?, last_minion=?, stone_cp=?, golden_dice=?, sweep_reset=? WHERE id=?");
			...
			stmt.setInt(44, pcd.getMinionEnergy());
			stmt.setInt(45, pcd.getLastMinion());
			stmt.setInt(46, pcd.getStoneCreativityPoint());
			stmt.setInt(47, pcd.getGoldenDice());
			stmt.setInt(48, pcd.getResetBoard());
	...
	public boolean saveNewPlayer(final PlayerCommonData pcd, final int accountId, final String accountName) {
		...
		try {
			...
			PreparedStatement preparedStatement = con
				.prepareStatement("INSERT INTO players(id, `name`, account_id, account_name, x, y, z, heading, world_id, gender, race, player_class, quest_expands, npc_expands, warehouse_size, bonus_title_id, is_archdaeva, wardrobe_slot, online, stamps, rewarded_pass) "
						+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 2, 0, 0, 1)");
