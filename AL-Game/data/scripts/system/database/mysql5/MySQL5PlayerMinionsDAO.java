...
public class MySQL5PlayerMinionsDAO extends PlayerMinionsDAO {
	...
	public void insertPlayerMinion(MinionCommonData minionCommonData) {
		...
		try {
			...
			PreparedStatement stmt = con.prepareStatement("INSERT INTO player_minions(player_id, object_id, minion_id, name, grade, level, birthday, growthpoints, despawn_time, expire_time) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			stmt.setInt(8, minionCommonData.getMinionGrowthPoint());
			stmt.setTimestamp(9, minionCommonData.getDespawnTime());
            stmt.setInt(10, minionCommonData.getExpireTime());
			stmt.execute();
			stmt.close();
		}
		catch (Exception e) {
	...
	@Override
	public void removePlayerMinion(Player player, int minionObjectId, Timestamp birthday) {
		Connection con = null;
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("DELETE FROM player_minions WHERE player_id = ? AND minion_id = ? AND birthday=?");
	...
	public List<MinionCommonData> getPlayerMinions(Player player) {
		...
		try {
			...
			while (rs.next()) {
				MinionCommonData minionCommonData = new MinionCommonData(rs.getInt("minion_id"), player.getObjectId(), rs.getString("name"), rs.getString("grade"), rs.getInt("level"), rs.getInt("growthpoints"), rs.getInt("expire_time"));
				minionCommonData.setName(rs.getString("name"));
				minionCommonData.setGrade(rs.getString("grade"));
				minionCommonData.setMinionLevel(rs.getInt("level"));
				minionCommonData.setMinionGrowthPoint(rs.getInt("growthpoints"));
	...
	public void updateMinionName(MinionCommonData minionCommonData) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE player_minions SET name = ? WHERE player_id = ? AND minion_id = ? AND birthday=?");
	...
	@Override
	public void updatePlayerMinionGrowthPoint(MinionCommonData minionCommonData) {
		Connection con = null;
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE player_minions SET growthpoints = ? WHERE player_id = ? AND minion_id = ? AND birthday=?");
	...
	public boolean PlayerMinions(int playerid, int minionObjId) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				if(rs.getInt("minion_id") == minionObjId) return true;
			}
			stmt.close();
		}
		catch (Exception e) {
	...
	public void evolutionMinion(Player player, int minionId, MinionCommonData minionCommonData) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("UPDATE player_minions SET minion_id = ?, growthpoints = 0 WHERE player_id = ? AND birthday=?");
			stmt.setInt(1, minionId);
			stmt.setInt(2, player.getObjectId());
			stmt.setTimestamp(3, minionCommonData.getBirthdayTimestamp());
			stmt.execute();
			stmt.close();
		}
		catch (Exception e) {
	...
	public void lockMinions(Player player, int minionObjId, int isLocked) {
        ...
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("UPDATE player_minions SET is_locked = ? WHERE player_id = ? AND minion_id = ?");
	...
	@Override
    public void saveDopingBag(Player player, int minionId, MinionDopingBag bag) {
        Connection con = null;
        try {
            con = DatabaseFactory.getConnection();
            PreparedStatement stmt = con.prepareStatement("UPDATE player_minions SET buff_bag = ? WHERE player_id = ? AND minion_id = ?");
            ...
            log.error("Error update doping for minion #" + minionId, e);
        } finally {
            DatabaseFactory.close(con);
        }
    }
	public void saveBirthday(MinionCommonData minionCommonData) {
		...
		try {
			con = DatabaseFactory.getConnection();
			PreparedStatement stmt = con.prepareStatement("SELECT * FROM player_minions WHERE player_id = ? AND minion_id = ? AND birthday=?");
			stmt.setInt(1, minionCommonData.getMasterObjectId());
