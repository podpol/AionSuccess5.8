...
public final class PlayerEnterWorldService
{
	...
	static {
		String infoBuffer = "Announce: Welcome at " + GSConfig.SERVER_NAME + " server.\n";
		infoBuffer = infoBuffer + "=============================\n";
		infoBuffer = infoBuffer + "Info: Type .faction to use the chat for your faction.";
		infoBuffer = infoBuffer + "=============================\n";
		String alBuffer = "=============================\n";
		alBuffer = alBuffer + "This emu is developed by Aion.\n";
		alBuffer = alBuffer + "Copyright 2009 - 2019 Aion\n";
		if (GSConfig.SERVER_MOTD_DISPLAYREV) {
			alBuffer = alBuffer + "=============================\n";
			alBuffer = alBuffer + "Server Revision: " + String.format("1-1 F.R", new Object[] { new Version(GameServer.class).getRevision() }) + "\n";
		}
		alBuffer = alBuffer + "=============================\n";
	...
	public static final void enterWorld(AionConnection client, int objectId) {
		...
		if (player != null && client.setActivePlayer(player)) {
			...
			*/
			...
			//Color Chat.
			PacketSendUtility.sendBrightYellowMessageOnCenter(player, ColorChat.colorChat("Welcome to RexusAion", "1 0 5 0"));
	...
	public static final void LoginServerInfo(Player player) {
		...
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300564, "[color:Info：;0 1 0] Welcome to Rexus-Aion Emulator"));
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300564, "[color:Info：;0 1 0] (Bugs & Suggestions) go in Facebook Group."));
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300564, "============================"));
		PacketSendUtility.sendPacket(player, new SM_SYSTEM_MESSAGE(1300564, "[color:Info：;0 1 0] Rexus Aion Account Information"));
